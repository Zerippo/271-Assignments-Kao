...
public class SimpleLinkedList {
    ...
    public Node findMiddle() {
        Node slow = head;// sets both pointers to the first node
        Node fast = head;

        if (head == null) {
            slow = null; // if Empty list, return null
        } else {
            while (fast.next != null && fast.next.next != null) { // as long as the the next two nodes after fast are not null
    ...
    public SimpleLinkedList invert() {
        ...
        invertedList.head = previous; // Update head of the new list
        return invertedList;
